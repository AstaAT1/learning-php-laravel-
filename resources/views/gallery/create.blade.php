...
<div class="min-h-[80vh] bg-zinc-950 px-4 py-10">
  <div class="max-w-2xl mx-auto">
    ...
    <div class="mb-8">
      <p class="text-amber-400 text-xs font-mono tracking-widest uppercase mb-2">— Upload</p>
      <h1 class="text-white text-4xl font-bold tracking-tight">Add Photo</h1>
      ...
      <form action="{{ route('gallery.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
          ...
          >
            ...
            <option value="Pets" {{ old('type') == 'Pets' ? 'selected' : '' }}>Pets</option>
            <option value="Landscape" {{ old('type') == 'Landscape' ? 'selected' : '' }}>Landscape</option>
            <option value="Portrait" {{ old('type') == 'Portrait' ? 'selected' : '' }}>Portrait</option>
